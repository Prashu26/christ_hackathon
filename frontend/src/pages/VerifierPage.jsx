...
function VerifierPage() {
  ...
  const startScanning = async () => {
    try {
      ...
      setError('Camera access denied. Please allow camera permission or upload a QR code image.')
    }
  }
  ...
  const renderVerificationResult = () => {
    ...
    return (
      <div className={`mt-8 p-6 rounded-xl text-center ${
        isValid ? 'bg-green-500/20 border border-green-500/50' : 'bg-red-500/20 border border-red-500/50'
      }`}>
        ...
        {isValid && credentialData && (
          <div className="bg-white/10 rounded-lg p-4 mt-4 text-left">
            ...
            <p><strong>Type:</strong> {credentialData.type}</p>
            <p><strong>Mode:</strong> {credentialData.mode}</p>
            <p><strong>Issued:</strong> {new Date(credentialData.timestamp).toLocaleString()}</p>
            <p><strong>Expires:</strong> {new Date(credentialData.expiresAt).toLocaleString()}</p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-8 text-white">
      ...
      <div className="text-center mb-12 pt-16">
        ...
        <p className="text-lg opacity-90">Scan a QR code to verify user credentials instantly</p>
      </div>
          ...
          <div>
            ...
            {cameraPermission === false && (
              <div className="p-6 bg-red-500/20 border border-red-500/50 rounded-xl">
                <p className="mb-2">Camera access is required for QR code scanning.</p>
                <p>Please allow camera permission or upload an image instead.</p>
              </div>
            )}
          </div>
        )}
